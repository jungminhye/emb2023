...
const char cnt_note [] =
"[#]"
"F4 P F4 P F4 P F+4 P"
;

const char song_note [] =
"[#]"
"F4 F4 A4 A8 B@8 B@8 A8 B@4 D+8 C+4 B@8 F6 G6"
"A8 F4 G8 F4 B4 A B8 D+8 C+2"
"F4 F4 A4 B@8 B@8 A8 B@8 D+8 C+4 B8 F6 G6 A8 F4 A8 F+8 E+8 C+8 C+8 F+8 E+8 C+8 C+2"
"F4 F4 A4 A8 B@8 B@8 A8 B@8 D+8 C+4 B@4 A8 F4 F8 F+8 E+8 C+8 C+8 A8 B@8 C+8 C+2"
"F4 F4 A4 A8 B@8 B@8 A@8 B@8 D+8 C+4 B@4 A8 F4 F8 F+8 E+8 C+8 C+8 A8 B@8 C+8 C+2"
"C++8 B+@8 A+8 B+@6 A+6 A+8 A+8 G+8 F+8 F+8 F+8 F+8 F+8 E+4 F+8 G+8 G+8 A+8 F+8 C+8 A8 B@8 C+8 B8"
;
...
const char gBuzzerBaseSysDir[1024];
...
int buzzerInit(void)
{
	if(findBuzzerSysPath())		//부저 경로를 받아오지 못했다면
	{
	...
	fdEn = open(path,O_WRONLY);		//enable 
	if(fdEn<=0)
	{
	...
	fdFq = open(path,O_WRONLY);		//frequency
	if(fdFq<=0)
	{
	...
	while ( 1 )
	{
		...
		{	//매 틱마다.
			...
			if (waitToNextTick == 0)
			{
				...
				{
					while ( songPtr <= strlen(cnt_note) ) //while of Parse.
					{
						if (parseType == PARSE_TYPE_INITIAL)	//이것이 뭔가?!
						{
							//if ( (song_note[songPtr] == 0) || (song_note[songPtr] == ' ')	songPtr++;	//이건 공백은 해석하지 않기.

							switch (cnt_note[songPtr])
							{
								case 'A': newNote = 21;	parseType = PARSE_TYPE_NOTE;	parseStage = PARSE_STAGE_OCT;	nearByScale = 'A'; songPtr++; break;
								case 'B': newNote = 23;	parseType = PARSE_TYPE_NOTE;	parseStage = PARSE_STAGE_OCT; nearByScale = 'B'; songPtr++; break;
								case 'C': newNote = 12;	parseType = PARSE_TYPE_NOTE;	parseStage = PARSE_STAGE_OCT;	nearByScale = 'C'; songPtr++; break;
						...
						else if (parseType == PARSE_TYPE_NOTE)
						{
							...
							if (parseStage == PARSE_STAGE_OCT)
							{
								if (cnt_note[songPtr] == '+')		//높은 옥타브...
								{
									//printf ("HighOct!\r\n");
									newNote+=12;
									songPtr++;
								}
								else if (cnt_note[songPtr] == '-')	//낮은 옥타브...
								{
									newNote-=12;
									songPtr++;
								}
							...
							else if (parseStage == PARSE_STAGE_SUSTAIN)
							{
								...
								if (cnt_note[songPtr] == '1')
								{
									waitToNextTick = DEFAULT_SUS*32;	//32가 minimum Resolution.
									songPtr++;
								}
								else if (cnt_note[songPtr] == '2')
								{
									waitToNextTick = DEFAULT_SUS*16;	//32가 minimum Resolution.
									songPtr++;
								}
								else if (cnt_note[songPtr] == '4')
								{
									waitToNextTick = DEFAULT_SUS*8;	//32가 minimum Resolution.
									songPtr++;
								}
								else if (cnt_note[songPtr] == '8')
								{
									waitToNextTick = DEFAULT_SUS*4;
									songPtr++;
								}
								else if (cnt_note[songPtr] == '6')
								{
									waitToNextTick = DEFAULT_SUS*2;
									songPtr++;
								}
						...
						else if (parseType == PARSE_TYPE_MAJOR)
						{
							...
							{
								//printf ("%c!\r\n",song_note[songPtr]);
								if ( cnt_note[songPtr] >= 'A' && cnt_note[songPtr] <= 'G' )
								{
									permaMajor[cnt_note[songPtr] - 'A'] = majorMinor;
									songPtr++;
								}
								else
								{
